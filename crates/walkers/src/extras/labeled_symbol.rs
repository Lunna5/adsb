...
impl Place for LabeledSymbol {
    ...
    fn draw(&self, ui: &Ui, projector: &crate::Projector) {
        if !projector.is_in_view(self.position) {
            return; // Skip drawing if the position is not in view.
        }

        profiling::scope!("LabeledSymbol::draw");
...
impl Group for LabeledSymbolGroup {
    fn draw<T: Place>(
        ...
        if !projector.is_in_view(position) {
            return; // Skip drawing if the position is not in view.
        }
